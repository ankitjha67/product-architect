...
`frameworks/enterprise-edge-cases.md` holds the generic shocks. This section is the recruiting
counterpart: the ways a hiring machine breaks for reasons that have nothing to do with candidate
quality. Recruiting sits downstream of budget, org design, legal entity and comp, which means it
absorbs every one of their shocks with a candidate on the other end of the phone.

| Edge case | Trigger / how you notice | What actually happens | The move |
|---|---|---|---|
| **Hiring freeze lands after offers are extended** | Finance announces a freeze, or req approvals silently stop; you have signed offers with start dates inside 30 days | Rescinding a signed offer is a legal question in some jurisdictions and an employer-brand event in all of them, and the candidate has usually resigned elsewhere. One rescinded offer reaches a public forum within days and prices every future close | Agree the offer-in-flight rule with [agents/18-finance.md](18-finance.md) and [agents/22-people-hr.md](22-people-hr.md) BEFORE any freeze is announced. Default: honour signed offers, pause unopened reqs, and hold verbal-offer-not-yet-papered cases as an explicit decision with a named approver |
| **The req is approved in one cost centre and needed in another** | Post-reorg, the hiring manager's team moved but the position ID and budget line did not | The req is worked for weeks and then cannot be offered against, because the funding entity no longer owns the role. At 5,000-plus people this is the single most common silent stall | No req opens without a position ID, a budget line and a level (section 8 of the enterprise-grade block). Re-validate all open reqs against the new cost-centre map within two weeks of any reorg |
| **Agency fee dispute over candidate ownership** | Two agencies submit the same candidate, or a candidate applied directly six weeks before an agency submitted them | A fee claim of 15 to 30 percent of first-year comp that you may owe twice, or a candidate you cannot hire cleanly. Terms nobody read in an agreement signed by a manager, not by procurement | One master agency agreement per vendor via [agents/46-procurement-supply-chain.md](46-procurement-supply-chain.md), with a defined ownership window (commonly 6 to 12 months, verify current terms), submission by ATS only, and a hard rule that no manager signs agency terms |
| **Candidate data retention becomes a privacy finding** | A subject access or erasure request, or an audit that finds 400,000 CVs in the ATS with no purge date | Under GDPR and comparable regimes you must state a retention period and honour it. Talent-pool data held on consent that was never refreshed is unlawful to keep, and a rejected candidate is more likely to complain than a hired one | Automated purge configured in the ATS, retention stated in the candidate notice, talent-pool consent refreshed on a schedule, and the whole design signed off by [agents/39-privacy-dpo.md](39-privacy-dpo.md). Intention is not a retention policy |
| **The referral pipeline quietly narrows diversity** | Referral share climbs past 40 percent while pipeline demographics flatten quarter over quarter | Referrals reproduce the composition of the existing team by arithmetic, so the fastest, highest-quality channel is also the one that makes the team more homogeneous every quarter. Nobody notices until an external report does | Cap channel share at the plan level rather than discounting referrals, run targeted referral drives, and report pipeline demographics by stage AND by channel. Fix the process gap if pass-through diverges; sourcing spend cannot fix a process problem |
| **Interview loop capacity collapses when the org is busy** | Scheduling latency doubles, panels decline invitations, feedback arrives at day four instead of day one | A production incident, a launch or a crunch removes engineer-hours from the funnel exactly when hiring is meant to accelerate. Median top-tier candidates are off market in about 10 days, so a scheduling lag rejects them on your behalf | Model panel hours as a real constraint (section 1) and get them committed in writing per quarter. Have a named backup panel per competency, and when capacity drops, reduce the number of live reqs rather than the interview standard |
| **A competing internal transfer for the same candidate** | Two hiring managers, one internal applicant; or an internal candidate applies while their manager is planning a promotion for them | Handled badly this burns two relationships and the internal candidate leaves entirely. Handled by whoever escalates loudest, it teaches the org that internal mobility is political | A published internal-mobility policy: the candidate's choice governs, the current manager is informed at a defined stage and cannot veto, and release timing is negotiated on a fixed default (commonly 4 to 8 weeks). Owned with [agents/22-people-hr.md](22-people-hr.md) |
| **Background or reference check returns adverse after start date** | The vendor is slow, the candidate started on a conditional basis, or a regulated check clears late | Withdrawing employment after start is a termination in most jurisdictions, with notice, process and possibly consultation. In FCRA territory, adverse action requires a pre-adverse notice and a waiting period | Checks are a gate before the start date, never a parallel track. If the business insists on an early start, get written acknowledgement of the conditional basis reviewed by counsel, and never do this for regulated roles |
| **The offer letter promises equity that was never approved** | Diligence, an audit, or the first exercise request surfaces grants that no board or NRC minute covers | A real liability and a diligence finding, plus an employee who was promised something you cannot deliver. Recruiters improvising equity numbers is the usual root cause | Equity numbers come only from [agents/61-total-rewards.md](61-total-rewards.md) bands, and every offer states that grants are subject to board approval. Reconcile offers issued against grants approved every quarter |
| **Pay transparency forces a range you have not rationalised** | A new posting requirement in a state or country where you have live reqs, or a candidate quoting your own posted range back at you | Posting a range exposes an internal band that may not be defensible, and existing employees read the posting too. The first casualty is usually an employee paid below the range you just published for their own job | Never post a range that has not been reconciled against current incumbents. Route the reconciliation to [agents/61-total-rewards.md](61-total-rewards.md) before the posting goes live, and expect a correction cost |
| **Two business units bid for the same candidate at different bands** | A candidate in two loops, or an internal escalation about who "owns" the pipeline | Whoever concedes a level wins the candidate and reprices the level for everyone. Level inflation via recruiting is permanent and expensive | One candidate record, one owner, one band. Cross-BU conflicts route to a named arbiter, not to negotiation. The level is set by the rubric, never by which manager needed the hire more |
| **Volume cohort arrives with no onboarding or training capacity** | A 60-person cohort start date set from the hiring plan, not from the training calendar | Cohort attrition inside 90 days spikes, and the hires that survive ramp slowly and badly. The recruiting metric looks excellent and the business outcome is worse than not hiring | Cohort start dates are set by training and manager capacity with [agents/23-learning-development.md](23-learning-development.md). Model attrition-adjusted plan: at 25 percent 90-day attrition, hiring to plan is hiring roughly a third short of it |
| **A regulated-sector hire cannot be cleared** | Fit-and-proper, licensing, sanctions or exclusion-list screening comes back blocked, sometimes weeks after acceptance | The person cannot lawfully perform the role. In financial services and healthcare this is not negotiable, and the regulator may want to know how they were nearly onboarded | Regulated screening runs as a pre-start gate with evidence retained for [agents/59-internal-audit-risk.md](59-internal-audit-risk.md). Time it into the offer-to-start window and tell the hiring manager the real date, not the optimistic one |
| **An employer-brand event lands mid-search** | A layoff, a viral rejection, a public incident, or a review-site pile-on while 30 reqs are live | Reply rates on outbound drop, accept rates fall, and candidates raise it in the loop. Recruiters improvise answers and each one is different | Agree a single factual line with [agents/25-pr-communications.md](25-pr-communications.md) within 48 hours and brief every recruiter and panellist on it. Do not pretend it did not happen; candidates have already read it |
| **ATS migration loses the audit population** | Cutover; the new system has open reqs and current candidates, and not much history | Rejection reasons, scorecards, demographic pass-through and consent records are exactly the population a regulator, plaintiff or auditor samples. Losing them removes your defence, not just your reporting | Define the retained-history requirement from the compliance use case before vendor selection, keep an immutable export of the old system for the retention period, and reconcile hire counts against the HRIS before decommissioning |
| **Executive search leaks and the incumbent finds out** | A board member mentions it, a search firm calls someone inside the company, or a candidate posts about it | The incumbent's authority collapses immediately, their team starts interviewing, and the search now runs against a clock nobody planned for | Confidential searches use a retained firm with a written confidentiality protocol, a named-population list of who knows, and a pre-agreed plan for what happens on day one of a leak. Coordinate with [agents/22-people-hr.md](22-people-hr.md) and [agents/62-chief-of-staff-bizops.md](62-chief-of-staff-bizops.md) |
| **The req is approved for a country where the entity does not exist** | Offer stage; payroll asks which entity employs this person | Either an EOR arrangement nobody budgeted (roughly a few hundred dollars per employee per month on top of payroll, verify current pricing) or a hire that cannot legally start for months | Entity and employment route are decided at req approval, not at offer. Keep a live country list of where you can hire, how, and at what lead time, maintained with [agents/22-people-hr.md](22-people-hr.md) and [agents/57-tax.md](57-tax.md) |
| **An AI screening vendor becomes a regulatory liability mid-contract** | New obligations bite (NYC Local Law 144 bias audits, EU AI Act high-risk duties, Illinois AIVIA), or the vendor cannot produce an audit | You are using an automated employment decision tool you cannot evidence, on a population you have already rejected. Retroactive remediation is not available | Do not buy screening that auto-rejects. Where AI is used for search, scheduling or notes, keep the reject decision human and logged, require audit evidence as a contractual deliverable, and re-check obligations each renewal with [agents/11-compliance-ethics.md](11-compliance-ethics.md) |

**Failure modes specific to this function**
- **REQ WITHOUT A SPINE:** hiring starts before the position ID, budget line and level exist, so the stall arrives at offer stage when it is most expensive.
- **PIPELINE IN A PERSON:** the search lives in one recruiter's head and inbox; when they leave, four months of relationship-building leaves with them.
- **SPEED METRICS DRIVING THE PROCESS:** time-to-fill is the number leadership sees, so the loop shortens under pressure and quality-of-hire degrades 6 to 12 months later, invisibly.
- **COMPLIANCE AS POST-HOC CLEANUP:** rejection reasons, consents and demographic separation reconstructed after the fact, which is exactly when they carry no evidential weight.
- **CAPACITY PROMISED, NOT MODELLED:** panel hours agreed verbally in a planning meeting and never booked, so the funnel throttles at the interview stage every quarter.
- **RECRUITING ABSORBING OTHER FUNCTIONS' SHOCKS SILENTLY:** freezes, reorgs and band changes handled candidate-by-candidate instead of escalated as a plan-level problem.

**Escalation and who owns what**
- Headcount plan, org design, internal mobility, onboarding capacity: [agents/22-people-hr.md](22-people-hr.md)
- Bands, level rubric, offer exceptions, pay-transparency reconciliation, equity approval: [agents/61-total-rewards.md](61-total-rewards.md)
- Freeze policy, offer-in-flight rules, cost-centre and budget lines: [agents/18-finance.md](18-finance.md)
- Candidate data retention, consent, DPIA for any screening tool: [agents/39-privacy-dpo.md](39-privacy-dpo.md)
- Selection criteria, adverse impact, background-check scope, offer terms: [agents/10-legal-ip.md](10-legal-ip.md)
- Automated decision tools, regulated screening obligations: [agents/11-compliance-ethics.md](11-compliance-ethics.md)
- Agency master agreements, ATS and assessment vendor contracts: [agents/46-procurement-supply-chain.md](46-procurement-supply-chain.md)
- Employer-brand response during a public event: [agents/25-pr-communications.md](25-pr-communications.md)
- Cohort onboarding and training capacity: [agents/23-learning-development.md](23-learning-development.md)
- Audit evidence for hiring populations, regulated pre-employment checks: [agents/59-internal-audit-risk.md](59-internal-audit-risk.md)
- Entity availability, employment route, relocation tax: [agents/57-tax.md](57-tax.md)
- Confidential executive searches and conflicting executive mandates: [agents/62-chief-of-staff-bizops.md](62-chief-of-staff-bizops.md)

**Pre-mortem prompts for this department**
1. If a freeze landed on Friday, exactly which candidates would we have to call, and who decides what we tell them?
2. Which open reqs would survive a cost-centre remap tomorrow, and which would have no funding entity at all?
3. If the recruiter running our hardest search left next week, what would remain in the ATS versus in their head?
4. Can we produce, today, the selection criteria, scorecards, rejection reasons and consents for every candidate rejected in the last 12 months?
5. What percentage of our pipeline comes from the single largest channel, and what happens to hiring if that channel closes?
6. Have we reconciled every posted salary range against the people already doing that job?
7. Which of our hires this quarter had panel hours that were actually booked, rather than assumed?
8. For every country on the plan, do we know the entity, the employment route and the real lead time to a legal start date?
